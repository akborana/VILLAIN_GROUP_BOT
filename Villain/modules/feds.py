...
import Villain.modules.sql.feds_sql as sql
from Villain import DRAGONS, EVENT_LOGS, LOGGER, OWNER_ID, TIGERS, WOLVES, dispatcher
...
from Villain.modules.helper_funcs.chat_status import is_user_admin
from Villain.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from Villain.modules.helper_funcs.string_handling import markdown_parser

# Hello bot owner, I spended for feds many hours of my life, Please don't remove this if you still respect Mr unknown  and Joker  too
# Federation by MrJoker 2018-2019
# Federation rework by Unknown 2019")
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == "":
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @Akborana if the problem persist."
            )
            return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @Akborana should this problem persist!"
            )
            return
...
def set_frules(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @AKBORANA !"
            )
            return
