...
from Villain.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if user_id == bot.id:
        message.reply_text("Oh Bhai maro muje maro, ban myself, what the fuck!")
        return log_message
...
def sban(update: Update, context: CallbackContext) -> str:
    ...
    if user_id == 948247711 or user_id == 1541292599:
        return log_message
...
def kick(update: Update, context: CallbackContext) -> str:
    ...
    if res:
        ...
        bot.sendMessage(
            chat.id,
            f"User Kicked PARTY! {mention_html(member.user.id, html.escape(member.user.first_name))}.",
            parse_mode=ParseMode.HTML,
        )
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
